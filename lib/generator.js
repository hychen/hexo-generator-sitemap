...
module.exports = function(locals) {
  ...
  var homepages = util.buildHomePages(config.url, supportedLangs);
  ...
  var posts = homepages.concat(locals.posts.toArray(), locals.pages.toArray())
    ...
    .map(function(post) {
      ...
      //@TODO: convert languages to ISO 639-1 + ISO 3166-1 Alpha 2.
      post.lang = supportedLangs.indexOf(mlang) >= 0 ? mlang.split('-')[0].toLowerCase() : defaultLang;
      return post;
    })
    .sort(function(a, b) {
  ...
  var xml = template().render({
    config: config,
    posts: posts,
    alts: util.groupByAlternatives(supportedLangs, posts),
